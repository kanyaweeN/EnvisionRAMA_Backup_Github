...
public partial class OnlineMessageBox : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (destination == "ManualPopup")
                setControlMessageBox();
            else if (destination == "IsBusy")
                setBinddingControl("ไม่สามารถแก้ไขนัดได้ในขณะนี้", "1", "ตกลง", "", "", "warning");
            else if (destination == "DeleteSchedule")
                setBinddingControl("Xray ยืนยันข้อมูลแล้ว กรุณาโทรแจ้งถ้าต้องการยกเลิก", "1", "ตกลง", "", "", "warning");
            else if (destination == "Holiday")
                setBinddingControl("วันที่เลือกตรงกับวันหยุด ต้องการดำเนินการต่อหรือไม่", "2", "ตกลง", "ยกเลิก", "", "warning");
            else
    ...
    private void setBinddingControl(string txtMessage, string noOfButton, string cap1, string cap2, string cap3, string alttype)
    {
        ...
        switch (alttype)
        {
            case "W":
                ...
                break;
            case "warning":
                imgAlert.ImageUrl = "../../Resources/ICON/MessageBoxs/warning.png";
                break;
            case "flag":
                imgAlert.ImageUrl = "../../Resources/ICON/MessageBoxs/flag32.png";
                break;
            default:
    ...
    private void setReturnValue()
    {
        ...
        if (is_multipopup)
            ...
            ClientScript.RegisterStartupScript(Page.GetType(), "mykey", "CloseAndRebind('" + Request.QueryString["FROM"].ToString() + "');", true);
    }
}
