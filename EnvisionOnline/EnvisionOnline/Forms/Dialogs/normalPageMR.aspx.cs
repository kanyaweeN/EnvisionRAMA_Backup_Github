...
public partial class normalPageMR : System.Web.UI.Page 
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            checkIsValue();
            List<string> ListPageMR = new List<string>();
        }
    }
    ...
    protected void chekbox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        List<string> ListPageMR = Session["ListExamDtl"] as List<string>;
        string[] sptexam_uid = clickedButton.Value.Split('_');

        //DataTable dt = new DataTable();
        //foreach (string strExamUID in sptexam_uid)
        //{
        //    DataTable dtAllExam = Application["ExamAllData"] as DataTable;
        //    DataRow[] getCol = dtAllExam.Select("EXAM_UID = '" + strExamUID + "'");
        //    int getID = Convert.ToInt32(getCol[0]["EXAM_ID"]);

        //    ScheduleClass schedule = new ScheduleClass();
        //    if (dt.Rows.Count == 0)
        //        dt = schedule.get_ScheduleConflictExam(getID, param.REG_ID);
        //    else
        //        dt.Merge(schedule.get_ScheduleConflictExam(getID, param.REG_ID));
        //}
        ...
        foreach (string strExamUID in sptexam_uid)
        {
            if (clickedButton.Checked)
            {
                ...
                //    else
                //    {
                //        addExamDetail(strExamUID);
                //        ListPageMR.Add(strExamUID);
                //    }
                //}
                //catch
                //{
            ...
            {
                removeExamDetail(strExamUID);
                ListPageMR.Remove(strExamUID);
            }
        }
    }
