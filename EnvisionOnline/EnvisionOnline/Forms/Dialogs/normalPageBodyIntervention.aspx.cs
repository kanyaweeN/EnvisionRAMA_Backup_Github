...
public partial class normalPageBodyIntervention : System.Web.UI.Page 
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            checkIsValue();
            //List<string> ListPageBodyIntervention = new List<string>();
            //Session["ListExamDtl"] = ListPageBodyIntervention;
        }
    ...
    protected void chekbox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        //List<string> ListPageBodyIntervention = Session["ListExamDtl"] as List<string>;
    ...
    protected void rtoolResult_ButtonClick(object sender, RadToolBarEventArgs e)
    {
        ...
        if (e.Item.Value == "btnSave")
        {
            ...
            param.FlagChecked = "N";
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "mykey", "CloseAndRebind('checkAppointment');", true);
        }
        ...
        foreach (Control c in Page.Controls)
        {
            foreach (Control childc in c.Controls)
            {
                if (childc is RadButton)
                {
                    ...
                    RadButton chk = (RadButton)childc;
                    DataRow[] rowCheck = param.dvGridDtl.Select("EXAM_UID='" + _value + "'");
                    if (rowCheck.Length > 0 && chk.Checked == false)
                    {
                        removeExamDetail(_value);
                    }
                }
            }
        }
        ...
        switch (e.Argument)
        {
            ...
            case "ShowalertExam":
                ...
                if (Session["MessageBoxAlertexamValue"].ToString().ToLower() == "save")
                {
                    List<string> ListPageBodyIntervention = Session["ListExamDtl"] as List<string>;
                    addExamDetail(clickedButton.Value);
                    ListPageBodyIntervention.Add(clickedButton.Value);
                }
                else
                {
