...
public partial class OnlineMessageBox2 : System.Web.UI.Page 
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (destination == "RiskManagement")
                if (Session["strRisk"] != null)
                {
                    string str = Session["strRisk"].ToString();
                    setBinddingControl(str, "1", "ตกลง", "", "", "flag");
                }
                else
    ...
    private void setReturnValue()
    {
        ...
        {
            if (Request.QueryString["FROM"].ToString() == "RiskManagement")
                ClientScript.RegisterStartupScript(Page.GetType(), "mykey", "CloseAndRebind2('" + "RiskManagement" + "');", true);
            else
