...
public partial class OnlineMessageBoxBusy : System.Web.UI.Page
{
    ...
    private string txtBusy()
    {
        ONL_PARAMETER param = Session["ONL_PARAMETER"] as ONL_PARAMETER;
        string str = string.Empty;
        DataTable dtBusy = new DataTable();
        dtBusy = getBusyCase(param.ORDER_ID.ToString() != "0" ? "XRAYREQ" : "SCHEDULE",
                    Convert.ToInt32(param.ORDER_ID),
                    Convert.ToInt32(param.SCHEDULE_ID)
                    );
        if (Utilities.IsHaveData(dtBusy))
        {
            if (dtBusy.Rows.Count == 0) return str;
