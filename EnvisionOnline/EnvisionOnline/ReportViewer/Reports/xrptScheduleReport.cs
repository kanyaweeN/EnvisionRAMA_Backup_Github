...
{
    public partial class xrptScheduleReport : DevExpress.XtraReports.UI.XtraReport
    {
        private int schedule_id, org_id;
        private DataSet dsTemp;
        private string vender, printor_name;

        public xrptScheduleReport(GBLEnvVariable envv)
        {
            InitializeComponent();
            this.PrintingSystem.ShowMarginsWarning = false;

        }
        public xrptScheduleReport(int _schedule_id, int _org_id, string _printor_name)
        {
        ...
        public xrptScheduleReport(DataSet ds, int _org_id, string _printor_name)
        {
            InitializeComponent();
            this.PrintingSystem.ShowMarginsWarning = false;
            dsTemp = new DataSet();
        ...
        private void bindingData(DataSet ds)
        {
            ...
            for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
            {
                if (ds.Tables[1].Rows[i]["SERVICE_TYPE"].ToString() == "1")
                {
                    if (ds.Tables[1].Rows[i]["INS_TEXT"].ToString() != "" || ds.Tables[1].Rows[i]["HR_UNIT_INS"].ToString() != "")
                    {
                        ...
                        {
                            DataRow[] rowRemove = dtIns.Select("INS_TEXT=''");
                            if (rowRemove.Length > 0)
                                dtIns.Rows.Remove(rowRemove[0]);
                        }
                        dtIns.AcceptChanges();
                    }
                }
            ...
            this.lblPrintBy.Text = "Print By : " + printor_name;
