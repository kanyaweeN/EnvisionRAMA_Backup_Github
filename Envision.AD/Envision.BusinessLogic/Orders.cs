...
namespace Envision.BusinessLogic
{
    ...
    public class Order 
    {
        ...
        public DataView Ris_OrderImage
        {
            get { return dvOrderImage; }
            set { dvOrderImage = value; }
        ...
        private void fillOrderData()
        {
            ...
            if (processOrder.Result != null)
            {
                ...
                if (processOrder.Result.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        item.HAS_REQUEST_RESULT = true;
                        item.REQUEST_RESULT_DATETIME = Convert.ToDateTime(dr["REQUEST_RESULT_DATETIME"]);
                    }
                    #endregion
                }
                #endregion
        ...
        private void getOrderImage()
        {
            ...
            if (process.Result != null)
                if (process.Result.Tables.Count > 0)
                {
                    ...
                    dvOrderImage = dtOrderImage.DefaultView;
                }
        }
        ...
        public bool Invoke()
        {
            ...
            try
            {
                ...
                int regID = 0;
                if (patient.LinkDown == false && patient.DataFromHIS || patient.DataFromLocal)
                {
                    ...
                    pAddHIS.HIS_REGISTRATION.IS_FOREIGNER = patient.NON_RESIDENCE;
                    ...
                    pAddHIS.HIS_REGISTRATION.HL7_FORMAT = patient.HL7_Format;
                    pAddHIS.Invoke();
                ...
                //    pUpdateHIS.HIS_REGISTRATION.HL7_FORMAT = patient.HL7_Format;
                //    pUpdateHIS.Invoke();
                ...
                else
                {
                    ...
                    string sa = mode;
                    pHIS.HIS_REGISTRATION.HL7_FORMAT = patient.HL7_Format;
                    pHIS.Invoke();
                ...
                else if (item.ORDER_ID == -1)
                {
                    ...
                    foreach (DataRow dr in dtOrder.Rows)
                    {
                        ...
                        processAddDetails.RIS_ORDERDTL.BPVIEW_ID = dr["BPVIEW_ID"].ToString() == "" ? 0 : Convert.ToInt32(dr["BPVIEW_ID"]); ;
                        processAddDetails.RIS_ORDERDTL.COMMENTS = dr["COMMENTS"].ToString();
                        processAddDetails.RIS_ORDERDTL.HIS_SYNC = 'N';//SendBilling(processAddDetails.RIS_ORDERDTL.ACCESSION_NO);
                        if (string.IsNullOrEmpty(dr["PREPARATION_ID"].ToString()))
                        {
                ...
                else
                {
                    ...
                    foreach (DataRow dr in dtOrder.Rows)
                    {
                        if (dr["ORDER_ID"].ToString() != string.Empty)
                        {
                            ...
                            if (dr["BPVIEW_ID"].ToString() == string.Empty)
                                processUpdate.RIS_ORDERDTL.BPVIEW_ID = 0;
                            else
                                processUpdate.RIS_ORDERDTL.BPVIEW_ID = (int)dr["BPVIEW_ID"];
                            ...
                            if (dr["ACCESSION_NO"].ToString() == string.Empty)
                            {
                                ...
                                if (processUpdate.RIS_ORDERDTL.MODALITY_ID > 0)
                                {
                                    ...
                                    processUpdate.RIS_ORDERDTL.ACCESSION_NO = getAccessionNo(processUpdate.RIS_ORDERDTL.MODALITY_ID, item.REF_UNIT);
                                }
                                else
                                {
                ...
                _orderid = processAdd.RIS_ORDER.ORDER_ID;
                _refuid = processAdd.RIS_ORDER.REF_UNIT;
                ...
                try
                {
                    ...
                    new ScanImages().Invoke(patient.Reg_UID, 0, item.ORDER_ID, dvOrderImage);
                }
                catch { }
                #endregion
            ...
            if (retFlag)
            {
                if (mode == "New")
                {
                    try
                    {
                        EnvisionAIMCService.Service1 AIMCws = new EnvisionAIMCService.Service1();
                        AIMCws.SendHL7AIMC(item.ORDER_ID);
                    }
                    catch { }
                }
            }
        ...
        public void SendBilling_Intervent(int OrderID)
        {
            ...
            DataTable dtt = getData.Get_Orderdtl_Intervention(OrderID);
            //DataRow dr = dtt.Rows[0];

            DataRow[] rows = dtt.Select("SERVICE_TYPE=1");
        ...
        private void writeErrorLog(Exception ex) {
            try
            {
                if (!Directory.Exists(@"C:\EnvisionLogFile"))
                    Directory.CreateDirectory(@"C:\EnvisionLogFile");
                string fileName = "Order" + DateTime.Now.ToString("ddMMyyyyhhmmss") + ".txt";
                string message = "\r\n==========================================";
                message += "\r\n=      Error On : " + DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss");
                message += "\r\n==========================================";
                message += "\r\nSource : " + ex.Source;
                message += "\r\n\r\n\r\nMessage: " + ex.Message;
                File.AppendAllText(@"C:\EnvisionLogFile\" + fileName, message);
            }
            catch { 
            
            }
        }
        private void webService_inputOrderCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
