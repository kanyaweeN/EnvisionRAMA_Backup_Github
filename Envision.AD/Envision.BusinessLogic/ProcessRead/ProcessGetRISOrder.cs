...
namespace Envision.BusinessLogic.ProcessRead
{
	public class ProcessGetRISOrder : IBusinessLogic
	{
        ...
        public DataSet GetOrderByScan(string HN)
        {
            DataSet ds = new DataSet();
            RISOrderSelectData _proc = new RISOrderSelectData();
            _proc.RIS_ORDER.HN = HN;
            ds = _proc.GetOrderByScan();
            return ds;
        }
        public DataSet GetIsInvestigation(string ACCESSION_NO)
        {
            DataSet ds = new DataSet();
            RISOrderSelectData _proc = new RISOrderSelectData();
            ds = _proc.GetIsInvestigation(ACCESSION_NO);
            return ds;
        }
        public DataSet getOrderEditLookup(string HN)
        {
            DataSet ds = new DataSet();
            RISOrderSelectData _proc = new RISOrderSelectData();
            ds = _proc.getOrderEditLookup(HN);
            return ds;
        }
	}
}
