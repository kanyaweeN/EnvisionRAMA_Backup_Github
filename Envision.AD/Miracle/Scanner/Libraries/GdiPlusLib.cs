...
namespace Miracle.Scanner
{
    public class Gdip
	{
        ...
        public static bool SaveDIBAs( string picname, IntPtr bminfo, IntPtr pixdat )
		{
            ...
            string files = picname; 
		Guid clsid;
        if (!GetCodecClsid(files, out clsid))
			{
                MessageBox.Show("Unknown picture format for extension " + Path.GetExtension(files),
        ...
        st = GdipSaveImageToFile(img, files, ref clsid, IntPtr.Zero);
