...
{
    public partial class frmWorkBook : Envision.NET.Forms.Main.MasterForm
    {
        ...
        private void loadStudyDateData() 
        {
            ...
            dttData = AcademicManagement.GetAcademicWorkList(env.UserID, dtFrom.DateTime, dtTo.DateTime);
            //dttData = AcademicManagement.GetAcademicWorkList(7210, dtFrom.DateTime, dtTo.DateTime);
        ...
        {
            dttData = new DataTable();
            dttData = AcademicManagement.GetAcademicWorkList(env.UserID);
            //dttData = AcademicManagement.GetAcademicWorkList(7210);
            grdData.DataSource = dttData;
            setGridColumns();
            bindDataControl();
        }
        ...
        private void bindDataControl() {
            ...
            txtResult.Text = rowHandle["RESULT_TEXT_PLAIN"].ToString();
            txtStudy.Text = rowHandle["REPORT_TEXT"].ToString();
            //txtHN.EditValue = rowHandle["HN"].ToString();
            //txtPatient.EditValue = rowHandle["PATIENT_NAME"].ToString();
            //txtAcc.EditValue = rowHandle["ACCESSION_NO"].ToString();
            //txtGrade.EditValue = rowHandle["GRADE_VALUE"].ToString();
            //txtStatus.EditValue = rowHandle["STATUS"].ToString();

            //txtAge.EditValue = rowHandle["AGE"].ToString();
            //txtPriority.EditValue = rowHandle["PRIORITY"].ToString() == "R" ? "Routine" : rowHandle["PRIORITY"].ToString() == "S" ? "Stat" : rowHandle["PRIORITY"].ToString() == "U" ? "Urgent" : string.Empty;
            //txtExam.EditValue = rowHandle["EXAM_NAME"].ToString();
            //txtFinalizedBy.EditValue = rowHandle["FINALIZED_NAME"].ToString();
            //txtFinalizedOn.EditValue = rowHandle["FINALIZED_ON"].ToString();



            txtInfoHn.Text = rowHandle["HN"].ToString();
            txtInfoPatientName.Text = rowHandle["PATIENT_NAME"].ToString();
            string gender=rowHandle["GENDER"].ToString();
            ...
            if (dttSeverity != null)
            {
                DataRow[] rows = dttSeverity.Select("SEVERITY_ID= " + rowHandle["SEVERITY_ID"].ToString());
