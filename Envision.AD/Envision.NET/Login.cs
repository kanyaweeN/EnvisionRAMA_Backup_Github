        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Utilities.IsHaveData(ds))
                {
                    ...
                    {
                        string _name = ds.Tables[0].Rows[0]["name"].ToString().Trim();
                        string _fname = _name.Substring(0, _name.IndexOf(' ')).Trim();
                        string _lname = _name.Substring(_name.IndexOf(' ')).Trim();
                        ProcessAddHREmp addEmp = new ProcessAddHREmp();
                        addEmp.HR_EMP.EMP_UID = txtUser.Text.Trim();
                        addEmp.HR_EMP.PWD = Utilities.Encrypt(txtPassword.Text.Trim());
                        addEmp.HR_EMP.FNAME = _fname;
                        addEmp.HR_EMP.LNAME = _lname;
                        addEmp.HR_EMP.ORG_ID = 1;
                        addEmp.HR_EMP.CREATED_BY = 1;
                        addEmp.AddADFromHis();

                        ProcessGetGBLGrantRoleByEmployee getRole = new ProcessGetGBLGrantRoleByEmployee();
                        getRole.GBL_GRANTROLE.EMP_ID = addEmp.HR_EMP.EMP_ID;
                        if (!Utilities.IsHaveData(getRole.GetAD()))
                        {
                            ...
                            grantrole.ROLE_ID = 30;
                            grantrole.EMP_ID = addEmp.HR_EMP.EMP_ID;
                    ...
                    if (userInfo.IS_ACTIVE == 'Y')
                    {
                        ...
                        {
                            //msg.ShowAlert("SUP0001", new GBLEnvVariable().CurrentLanguageID);
                        }
                        #endregion
