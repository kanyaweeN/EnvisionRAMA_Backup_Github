...
{
    public class RISOrderInsertV2 :DataAccessBase
    {
        public RIS_ORDER RIS_ORDER { get; set; }

        public RISOrderInsertV2()
		{
            RIS_ORDER = new RIS_ORDER();
            //StoredProcedureName = StoredProcedure.Prc_RIS_ORDER_Insert_v2;
            StoredProcedureName = StoredProcedure.Prc_RIS_ORDER_Insert_v3;
		}
        ...
        private DbParameter[] buildParameter() {
            ...
            if(RIS_ORDER.IS_REQONLINE == string.Empty)
                pIS_REQONLINE.Value = DBNull.Value;
            else
                pIS_REQONLINE.Value = RIS_ORDER.IS_REQONLINE;

            DbParameter pENC_ID = Parameter();
            pENC_ID.ParameterName = "@ENC_ID";
            if (RIS_ORDER.ENC_ID == string.Empty || RIS_ORDER.ENC_ID == "0")
                pENC_ID.Value = DBNull.Value;
            else
                pENC_ID.Value = RIS_ORDER.ENC_ID;
            ...
            if (RIS_ORDER.IS_ALERT_CLINICAL_INSTRUCTION == string.Empty)
                pIS_ALERT_CLINICAL_INSTRUCTION.Value = DBNull.Value;
            else
                pIS_ALERT_CLINICAL_INSTRUCTION.Value = RIS_ORDER.IS_ALERT_CLINICAL_INSTRUCTION;

            DbParameter pCLINICAL_INSTRUCTION_TAG = Parameter();
            pCLINICAL_INSTRUCTION_TAG.ParameterName = "@CLINICAL_INSTRUCTION_TAG";
            if (RIS_ORDER.CLINICAL_INSTRUCTION_TAG == string.Empty )
                pCLINICAL_INSTRUCTION_TAG.Value = DBNull.Value;
            else
                pCLINICAL_INSTRUCTION_TAG.Value = RIS_ORDER.CLINICAL_INSTRUCTION_TAG;
			...
			DbParameter[] parameters ={
                ...
                ,pIS_REQONLINE, pENC_ID, Parameter("@ENC_TYPE", RIS_ORDER.ENC_TYPE), Parameter("@ENC_CLINIC", RIS_ORDER.ENC_CLINIC)
