...
namespace RIS.Forms.Schedule
{
	...
	public class frmMain : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.ultraMonthViewMulti1.MonthOrientation = Infragistics.Win.UltraWinSchedule.MonthOrientation.DownThenAcross;
		...
		private void ultraToolbarsManager1_ToolClick(object sender, Infragistics.Win.UltraWinToolbars.ToolClickEventArgs e)
		{
			switch ( e.Tool.Key )
			{
				...
				case "menuMain_View_Office2007_Silver":
				{
					...
					{
						Infragistics.Win.UltraWinSchedule.Owner owner = this.ultraCalendarInfo1.Owners[ownerKey];
		...
		{
            DateTime dtime = ultraCalendarInfo1.ActiveDay.Date;
           // this.pnlOwners.Controls.Clear();
            new GBLEnvVariable().ActiveDate = dtime;
           // this.ultraCalendarInfo1.Owners.
            ProcessModalityLoad procmload = new ProcessModalityLoad();
            procmload.Invoke(1, dtime);
            DataTable dtt = procmload.ResultSet.Tables[0];
            int j = 0;

            while( j<dtt.Rows.Count)
            {
                this.pnlOwners.Controls[j].Text = dtt.Rows[j]["Owner"].ToString();
                this.ultraCalendarInfo1.Owners[dtt.Rows[j]["MODALITY_ID"].ToString()].Name = dtt.Rows[j]["Owner"].ToString();
                j++;
            }
			...
			if ( e.Button == MouseButtons.Right )
			{
                ...
                Appointment ap=dayView.GetAppointmentFromPoint(cursorPos);
                if (ap != null)
                {
                    DatabaseSupport.UpdateAppointmentsTable();
                    MyMessageBox msg = new MyMessageBox();
                    string _id = msg.ShowAlert("UID0056", new GBLEnvVariable().CurrentLanguageID);
                    if (_id == "3")
                    {
                        
                        string hn=ap.ScheduleData.ToString();
                        DateTime appdt = ap.StartDateTime;
                        int modality = Convert.ToInt32(ap.OwnerKey.ToString());
                        int schedule = Convert.ToInt32(ap.Location.ToString());
                        RIS.Operational.DirectPrint.DirectPrint dp = new RIS.Operational.DirectPrint.DirectPrint();
                        dp.ScheduleDirectPrint(hn, appdt, modality,schedule);
                        return;
                    }
                    else if (_id == "2")
                    {
                        string hn2 = ap.ScheduleData.ToString();
                        DateTime appdt2 = ap.StartDateTime;
                        int modality2 = Convert.ToInt32(ap.OwnerKey.ToString());
                        int schedule2 = Convert.ToInt32(ap.Location.ToString());
                        RIS.Operational.ReportManager.ReportMangager reportprovider = new RIS.Operational.ReportManager.ReportMangager();
                        
                        RIS.Reports.ReportViewer.frmReportViewer RForm = new RIS.Reports.ReportViewer.frmReportViewer(reportprovider.rptScheduleReport(hn2,appdt2,modality2,schedule2),CloseControl);
                        RForm.Text = "Schedule Report";
                        RForm.Show();
                    }
                    else
                    {
...
//#endif
		...
		private void InitializeUI()
		{
            ...
            ownersDataBinding.KeyMember = "MODALITY_ID";
            ...
            appointmentsDataBinding.StartDateTimeMember = "APPOINT_DT";
            appointmentsDataBinding.EndDateTimeMember = "END_DATETIME";
            appointmentsDataBinding.ScheduleDataMember = "HN";
            appointmentsDataBinding.SubjectMember = "SCHEDULE_DATA";
            appointmentsDataBinding.RefFromMember = "BLOCK_REASON";
            appointmentsDataBinding.RefDoctorMember = "REF_DOC";
            appointmentsDataBinding.ScStatusMember = "SCHEDULE_STATUS";
            ...
            appointmentsDataBinding.OwnerKeyMember = "MODALITY_ID";
