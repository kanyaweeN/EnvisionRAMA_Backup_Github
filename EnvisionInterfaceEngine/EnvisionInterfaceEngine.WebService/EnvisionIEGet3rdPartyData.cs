...
namespace EnvisionInterfaceEngine.WebService
{
    ...
    public class EnvisionIEGet3rdPartyData : SoapHttpClientProtocol
    {
        ...
        public EnvisionIEGet3rdPartyData(string host) { Url = string.Format(url, host); }

        public void Set_TimeOut(int milliseconds) { Timeout = milliseconds; }
        ...
        [SoapDocumentMethodAttribute("http://www.miracleadvance.com/Check_Sync",
            RequestNamespace = "http://www.miracleadvance.com/",
            ResponseNamespace = "http://www.miracleadvance.com/",
            Use = SoapBindingUse.Literal,
            ParameterStyle = SoapParameterStyle.Wrapped)]
        ...
        {
            return ((object[])Invoke("Welcome", new object[] { }))[0].ToString();
        }
    }
}
