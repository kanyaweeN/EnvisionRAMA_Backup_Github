...
namespace Envision.NET.Forms.Main
{
    public partial class frmMain : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void bgPreload_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                Envision.NET.Forms.Orders.frmOrders ord = new Envision.NET.Forms.Orders.frmOrders();
                Envision.NET.Forms.ResultEntry.frmResultEntry result = new Envision.NET.Forms.ResultEntry.frmResultEntry();
            }
            catch (Exception ex)
            {
        ...
        private void bgPreload_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {

        }
        private void bgPreload_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            bgPreload.CancelAsync();
        }
        ...
        private void bgMsg_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                e.Result = dtt.Rows[0][0].ToString().Trim();
            }
            else
                e.Result = string.Empty;
        }
        private void bgMsg_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {

        }
        private void bgMsg_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (!string.IsNullOrEmpty(e.Result.ToString()))
            {
                int msg = Convert.ToInt32(e.Result.ToString());
                if (msg > no_ofMSG)
                {
                    ...
                    info.Image = Envision.NET.Properties.Resources.gmail_icon128;
