using System;
using System.Collections.Generic;
using System.Linq;
using Envision.Database;
using Envision.Entity.Common;

namespace Envision.DataAccess.Common
{
    public class DbACR
    {
        public DbACR(){
            dbItem = new HIS_ACR();
            Item = new ACR();
        }

        private HIS_ACR dbItem { get; set; }
        public ACR Item { get; set; }

        public int? Insert(){
            int? Id = null;
            using (var dbContext = new EnvisionDataModel())
            {
                dbItem = ConvertType(Item);
                dbItem.CREATED_ON = DateTime.Now;

                dbContext.HIS_ACR.Add(dbItem);
                dbContext.SaveChanges();

                Id = dbItem.ACR_ID;
            }
            return Id;
        }
        public bool? Update(){
            bool? flag = null;
            using (var dbContext = new EnvisionDataModel())
            {
                var obj = dbContext.HIS_ACR.Find(Item.Id);
                if (obj != null)
                {
                    
                    obj.ACR_ID = Item.Id;
                    obj.ACR_UID = Item.Code;
                    obj.ACR_DESC = Item.Description;
                    obj.ACR_VERSION = Item.Version;
                    obj.IS_UPDATED = "Y";
                    obj.IS_DELETED = Item.IsDelete.TryToString();
                    obj.ORG_ID = Item.OrgId;
                    obj.LAST_MODIFIED_BY = Item.ModifiedBy;
                    obj.LAST_MODIFIED_ON = DateTime.Now;

                    flag = dbContext.SaveChanges() > 0;
                }
            }
            return flag;
        }
        public bool? Delete() {
            bool? flag = null;
            using (var dbContext = new EnvisionDataModel())
            {
                var obj = dbContext.HIS_ACR.Find(Item.Id);
                if (obj != null)
                {
                    obj.IS_DELETED = "Y";
                    obj.LAST_MODIFIED_BY = Item.ModifiedBy;
                    obj.LAST_MODIFIED_ON = DateTime.Now;
                    flag = dbContext.SaveChanges() > 0;
                }
            }
            return flag;
        }

        public List<ACR> GetAllActiveData()
        {
            List<ACR> itemList = null;
            using (var dbContext = new EnvisionDataModel())
            {
                var itemSearch = (from data in dbContext.HIS_ACR
                                  where data.ORG_ID == Item.OrgId && data.IS_DELETED == "N"
                                  select data).ToList();
                if (itemSearch != null)
                {
                    itemList = new List<ACR>();
                    foreach (var obj in itemSearch)
                        itemList.Add(ConvertType(obj));
                }
            }
            return itemList;
        }
        public List<ACR> GetAllData()
        {
            List<ACR> itemList = null;
            using (var dbContext = new EnvisionDataModel())
            {
                var itemSearch = (from data in dbContext.HIS_ACR
                                  where data.ORG_ID == Item.OrgId
                                  select data).ToList();
                if (itemSearch != null)
                {
                    itemList = new List<ACR>();
                    foreach (var obj in itemSearch)
                        itemList.Add(ConvertType(obj));
                }
            }
             return itemList;
        }

        public ACR GetById()
        {
            ACR obj = null;
            using (var dbContext = new EnvisionDataModel())
            {
                var objFind = dbContext.HIS_ACR.Find(Item.Id);
                obj = ConvertType(objFind);
            }

            return obj;
        }
        public ACR GetByCode()
        {
            ACR obj = null;
            using (var dbContext = new EnvisionDataModel())
            {
                 var itemSearch = (from data in dbContext.HIS_ACR
                                  where data.ORG_ID == Item.OrgId
                                        && data.ACR_UID == Item.Code
                                  select data).ToList();
                if (itemSearch != null && itemSearch.Count > 0) {
                    obj = new ACR();
                    obj = ConvertType(itemSearch[0]);
                }
            }

            return obj;
        }

        #region Convert Type.
        private HIS_ACR ConvertType(ACR item)
        {
            HIS_ACR obj = null;
            if (item != null)
            {
                obj = new HIS_ACR();   

                obj.ACR_ID = item.Id;
                obj.ACR_UID = item.Code;
                obj.ACR_DESC = item.Description;
                obj.ACR_VERSION = item.Version;
                obj.IS_UPDATED = item.IsUpdate.TryToString();
                obj.IS_DELETED = item.IsDelete.TryToString();
                obj.ORG_ID = item.OrgId;
                obj.CREATED_BY = item.CreatedBy;
                obj.CREATED_ON = item.CreatedOn;
                obj.LAST_MODIFIED_BY = item.ModifiedBy;
                obj.LAST_MODIFIED_ON = item.ModifiedOn;
            }
            return obj;
        }
        private ACR ConvertType(HIS_ACR item)
        {
            ACR obj = null;
            if (item != null)
            {
                obj = new ACR();

                obj.Id = item.ACR_ID;
                obj.Code = item.ACR_UID;
                obj.Description = item.ACR_DESC;
                obj.Version = item.ACR_VERSION;
                obj.IsUpdate = item.IS_UPDATED.TryToBoolean();
                obj.IsDelete = item.IS_DELETED.TryToBoolean();

                obj.OrgId = item.ORG_ID;
                obj.CreatedBy = item.CREATED_BY;
                obj.CreatedOn = item.CREATED_ON;
                obj.ModifiedBy = item.LAST_MODIFIED_BY;
                obj.ModifiedOn = item.LAST_MODIFIED_ON;
            }
            return obj;
        }
        #endregion
    }
}